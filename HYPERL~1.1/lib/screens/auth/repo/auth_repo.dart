...
class AuthRepository {
  ...
  Future<List<AuthModel>> login({
    ...
    try {
      ...
      final response = await AppConstant.apiBaseHelper.postAPICall(ApiRoutes.loginApi, {
        if(email.isNotEmpty)
          'email': email,
  ...
  Future<List<AuthModel>> register({required String name, required String email, required String mobile, required String country, required String iso2, required String password, required String confirmPassword}) async {
    try {
      String? fcmToken = await getFCMToken();
      final response =
